...
## 3. Claude Code 설정 가이드 (03-setup.md)
...
/project/CLAUDE.md         → 프로젝트 팀 (git 커밋)
...
# 나쁜 예 (너무 많은 정보)
[50줄의 모든 가능한 명령어와 규칙...]
→ Claude가 관련 없는 정보 무시할 가능성 높아짐
```

#### 핵심 원칙
- **간결하게**: 컨텍스트는 제한된 자원
- **반복 개선**: 프롬프트처럼 튜닝
- **IMPORTANT 사용**: 중요한 규칙에 강조
- **# 키 활용**: 코딩 중 바로 메모 추가

### 3.2 MCP (Model Context Protocol) 설정
...
| 도구 | Stars | 분류 | 특징 |
|------|-------|------|------|
| **Claude Code** | 55K+ | CLI 에이전트 | Anthropic 공식, 200K 컨텍스트, 멀티파일 |
